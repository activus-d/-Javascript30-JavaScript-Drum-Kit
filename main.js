...
function playSound( e ) {
    ...
    //console.log(audio)
    if(!audio) return; //this means if the key pressed is not selected using the custom data attribute by querySelector, then do nothing
    ...
    if(!keyButton) return; //this means if the key button pressed is not selected using the custom data attribute by querySelector, then do nothing
...
function removeTransition(e) {
    //console.log(this) // notice the diffrence between the 'this' keyword and and the 'e' paramenter. 'this' represents the element the event is added 'transitionend' is added to
    console.log(e) //'e' parameter is the effect of transitionend event on each 'this'/element 
